...
def validate_vector(
    ...
    if b_size_of_a_must_be_eq_to_max_size and a.size != max_size:
        raise ValueError(
            f"The vector {name_of_a} has a size: {a.size}, but its size must be equal to {name_of_variable_with_equivalent_size}: {max_size}"
        )

    if a.size > max_size:
        raise ValueError(
            f"The vector {name_of_a} exceeds the maximum dimensions. It has size {a.size}, while the maximum allowed size is {max_size}"
        )
